...
[`../README.md`](../README.md) holds the one-screen map. The process is
adopted whole from the ecosystem's (soulidentity's `hq/`), because the
fold is run the same way the identity plane is.
...
- **Implementation follows a design.** A milestone on the roadmap points at
  the design doc (and decision numbers) it realizes; a capability that isn't
...
The normative design of the fold and its surfaces. Documents are written
functional-level — the capability, its seams, its configuration surface, its
acceptance criteria — with every load-bearing decision numbered and carrying
its reasoning. Every behavioral change made during implementation propagates
back into the design docs it touches — the docs describe the system as it
*is*. A new capability that isn't yet decided starts as research, not as a
design doc.
...
- **When to teach-back:** any decision that changes the authentication
  surface, the consumer seam, a scope, or a public claim. The assistant asks
  for the restatement; the decision is recorded only after it survives.
...
- **Adversarial pass:** for calls that change the authentication surface or
  the consumer seam, the assistant argues the other side at full strength
  *before* the decision.
...
- Gate: `make check` (fmt, tidy, build, test, lint) — all green, nothing
  ...
  (`go test ./...`) includes the hq structural lint.
- Keep pure logic (record shapes, lifecycle policy, claims assembly)
  separate from transport and storage so it unit-tests without a server; the
  e2e rigs are the one place real servers run.
- Sign every commit. Never commit `.claude/settings.local.json`.
- Passkeys-not-passwords (constitution I) and indistinguishable-by-design
